...
function choosevillain(type) {
	...
	if (type=='regular') {
		...
		$.ajax({
			url: "Champdelmundo.json",
			dataType: 'json',
			success: function (craig) {
				_this.sheet = craig;
			}
		});
	} else {
...
var defenses_update = function () {
	...
	window.game.player_live.defenses.fortitude = 10 + fortbonus + window.game.player_live.defensesbonus.fortitude;
...
function champfight() {
	choosevillain('champ');
	window.game.monster = new Opponent("Champdelmundo.json")
	//this will help to make sure the startgame button doesn't work.
	document.getElementById('gamelog').innerHTML="";
	console.log(window.game.monster);
	window.game.fighting = 1;
	window.game.location = 0;
	//add the necessary temp fields to the player.
	window.game.temp_player_att_bonus = 0;
	...
	document.getElementById("startturnbox").innerHTML="<button id='startturn' onclick='startturn()'>Start Your Turn!</button>"
	//these print to the screen.
	console.log("The Championship Match! The Brooklyn Brawler vs. "+window.game.monster.sheet.name)
	console.log(" ")
	console.log(" ")
	//these variables print the health to the healthbox.
	var monster_name = document.getElementById('monster_name_display');
...
function endfight(value) {
	...
	if (value == 'player') {
		...
		if (window.game.player.experience > 199) {
			console.log("You now have "+window.game.player.experience+" xp!  You can challenge for the title!  Do you want to find another fight or... go for the championship belt!?!?")
		} else {
	...
	if (window.game.player.experience > -1) {
		document.getElementById("startfightbox").innerHTML+="<button id='champfight' onclick=\"champfight()\">Challenge the Champ?</button>";
	}
...
FightGame.prototype.monsteraction = function() {
	...
	if (name == 'Vlad the Impaler') {
		...
		if (window.game.location == 1) {
			window.game.location = 0
			window.game.movetaken = 1
			console.log("Champ steps back and bounces off the ropes")
		}
		if (window.game.moves_library.sheet.m12.hit == 0) {
			...
			recharge = Math.floor(Math.random() * 6 + 1)
			if (recharge > 4){
				console.log("Do you smeeeeeell what the champ is cookin?!");
				var movechoice = "m12";
				name = window.game.moves_library.sheet.m9.moveName
			} else {
				var movechoice = "m13";
